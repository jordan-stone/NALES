...
def do_cds_plus(arr, bad_and_neighbors=None, vertical_overscan=True,
                jailbars_instead=False, method='cds'):
    """
    Apply correlated double sampling (or slope fitting) with additional corrections.

    This is the standard preprocessing for ALES raw frames:
    1. Collapse the ramp to a 2D image (CDS or slope fit)
    ...
    ----------
    arr : numpy.ndarray
        Raw 3D array (n_reads, ny, nx). For CDS, only the first and last
        reads are used. For slope fitting, all reads are used.
    ...
    method : str, optional
        Ramp collapse method. Default: 'cds'
        - 'cds': Correlated double sampling (last read - first read).
          Fast, removes reset noise, but ignores intermediate reads.
        - 'slopes': Fit a slope to all reads at each pixel.
          Uses all information in the ramp, reducing read noise by
          ~sqrt(n_reads) and enabling cosmic ray rejection.
          The output is in units of DN (scaled to match CDS, i.e.
          slope * (n_reads - 1)) so that downstream calibration
          is unchanged.
    ...
    processed : numpy.ndarray
        2D processed image.
    """
    arr0 = arr.copy().astype(np.float32)

    if method == 'cds':
        signal = arr0[-1] - arr0[0]
    elif method == 'slopes':
        signal = _fit_ramp_slopes(arr0)
    else:
        raise ValueError(f"Unknown method '{method}'. Use 'cds' or 'slopes'.")

    if jailbars_instead:
        ovscn2 = get_jailbars(signal)
    else:
        ovscn2 = get_overscan2(signal)

    out = signal - ovscn2[None, :]
...
def _fit_ramp_slopes(arr):
    """
    Fit per-pixel slopes to a non-destructive read ramp.

    Given an up-the-ramp sequence of n_reads frames, fits a line
    to the counts vs. read index at each pixel and returns the
    slope scaled to CDS-equivalent units: slope * (n_reads - 1).

    Parameters
    ----------
    arr : numpy.ndarray
        3D array of shape (n_reads, ny, nx), dtype float.

    Returns
    -------
    signal : numpy.ndarray
        2D slope image in CDS-equivalent DN.

    Notes
    -----
    Future improvements to consider:
    - Iterative sigma-clipping to reject cosmic ray hits
    - Handling of saturation (truncate the fit before saturated reads)
    - Returning a variance or chi-squared map alongside the slopes
    - Returning bias/reset frame if desired
    """
    n_reads = arr.shape[0]

    if n_reads < 2:
        raise ValueError(f"Need >= 2 reads for slope fitting, got {n_reads}")

    if n_reads == 2:
        # Degenerate case: slope fit is identical to CDS
        return arr[1] - arr[0]

    # --- Least-squares fit: y = m*x + b at every pixel ---
    x = np.arange(n_reads, dtype=float)
    A = np.column_stack([x, np.ones(n_reads)])

    ny, nx = arr.shape[1], arr.shape[2]
    Y = arr.reshape(n_reads, -1)

    result, _, _, _ = np.linalg.lstsq(A, Y, rcond=None)
    slope_per_read = result[0].reshape(ny, nx)

    # --- Analytic alternative (lower memory, faster) ---
    # If performance or memory is a concern, replace the lstsq block
    # above with the following:
    #
    # t = np.arange(n_reads, dtype=float)
    # sum_t = t.sum()
    # sum_t2 = (t ** 2).sum()
    # denom = n_reads * sum_t2 - sum_t ** 2
    #
    # sum_y = arr.sum(axis=0)
    # sum_ty = np.einsum('i,ijk->jk', t, arr)
    #
    # slope_per_read = (n_reads * sum_ty - sum_t * sum_y) / denom

    # Scale to CDS-equivalent units so downstream calibration is unchanged
    signal = slope_per_read * (n_reads - 1)
...
def organize_wavecal_frames(directory='.', maxNdarks=100, subtract_light_leak=True,
                            light_leak_output='light_leak_median.fits', method='cds'):
    """
    ...
    method : str, optional
        ...
        - 'cds': Correlated double sampling (last read - first read).
        - 'slopes': Fit a slope to all reads at each pixel, reducing
          read noise and enabling cosmic ray rejection. Requires
          raw frames with 3+ non-destructive reads to benefit over CDS.
    ...
    >>>
    >>> # Using slope fitting for improved noise performance
    >>> wave_cal_ims, light_leak = organize_wavecal_frames('/data/wavecal/',
    ...                                                     method='slopes')
    """
